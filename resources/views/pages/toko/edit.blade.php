<x-app-layout>
          ...
          <form method="post" action="{{ route('perpustakaan.update',$data['id']) }}">
            ...
            <div class="row">
              <div class="mb-3">
                ...
                <input id="nama_toko" name="nama_toko" type="text" class="form-control disabled" value="{{ $data['id'] }}" placeholder="Coffe Bar" readonly autocomplete="nama_toko">
              </div>
              <div class="mb-3">
                <label for="nama_toko" class="form-label fw-bold">{{ __('Nama Toko') }}</label>
                <input id="nama_toko" name="nama_toko" type="text" class="form-control" value="{{ $data['nama_toko'] }}" placeholder="Coffe Bar" required autocomplete="nama_toko">
              </div>
  ...
  <x-app-layout>
    ...
    <div class="py-12">
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-2">
          <form method="post" action="{{ route('pesanan.update',$data['id']) }}">
            @csrf
            @method('PUT')
            <div class="row">
              <div class="mb-3">
                <label for="nama_pesanan" class="form-label fw-bold">{{ __('Id Pesanan') }}</label>
                <input id="nama_pesanan" name="nama_pesanan" type="text" class="form-control disabled" value="{{ $data['id'] }}" placeholder="Mukena" readonly autocomplete="nama_pesanan">
              </div>
              <div class="mb-3">
                <label for="nama_pesanan" class="form-label fw-bold">{{ __('Nama Pesanan') }}</label>
                <input id="nama_pesanan" name="nama_pesanan" type="text" class="form-control" value="{{ $data['nama_pesanan'] }}" placeholder="Pakaian" required autocomplete="nama_pesanan">
              </div>
              <div class="col-span-6 sm:col-span-4">
                <label for="lokasi" class="form-label fw-bold">{{ __('Lokasi Toko') }}</label>
                <input id="lokasi" name="lokasi" type="text" class="form-control" value="{{ $data['lokasi'] }}" placeholder="Jl. Ahmad Yani, Blitar" required autocomplete="lokasi">
